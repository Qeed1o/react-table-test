...
// Mock API call для получения товаров
const mockFetchProducts = async (page: number, limit: number, search: string): Promise<{products: Product[], total: number}> => {
...
export const fetchProductsAsync = createAsyncThunk(
  ...
  async ({ page, limit, search }: { page: number; limit: number; search: string }) => {
    const response = await mockFetchProducts(page, limit, search);
    return response;
  }
);
